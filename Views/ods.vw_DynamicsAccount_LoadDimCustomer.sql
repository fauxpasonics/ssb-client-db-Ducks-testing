...
            , CAST(str_partnersalespersonname AS NVARCHAR (50)) AS [AccountRep] 
...
        --SELECT COUNT(*)           
        FROM ducks_reporting.prodcopy.account z WITH (NOLOCK) 
        WHERE 1=1
        AND (DATEDIFF(DAY,z.modifiedon,GETDATE())<=3 OR DATEDIFF(DAY,z.createdon,GETDATE())<=3) --This needs to be uncommented out after the first load, update the date fields
    ) a
 
)
 
