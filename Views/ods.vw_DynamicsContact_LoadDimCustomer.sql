...
-- Purpose: remapped emailaddress2 to emailtwo and emailaddress 3 to emailthree
...
-- Peer reviewed by: Standard Mapping
-- Peer review notes: Standard mapping
-- Peer review date: 20180601
-- Deployed by: 
-- Deployment date: 
-- Deployment notes: 
------------------------------------------------------------------------------- 
...
--base set
        SELECT
            'Ducks' AS [SourceDB] --Client Source Database such as Raiders or Raiders_Reporting
...
            , CAST(parentcustomeridname AS NVARCHAR (50)) AS [CompanyName] 
            , z.Salutation AS [SalutationName]
...
 
            /*AddressPrimary*/
            , NULLIF(ISNULL(address1_line1,''),'')  AS [AddressPrimaryStreet] --CASE WHEN dc.DimCustomerId IS NULL THEN  NULLIF(ISNULL(address1_line1,'') + ' ' + ISNULL(address1_line2,''),' ') ELSE address1_line1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
...
            , LEFT(telephone3,25) AS [PhoneOther]
 
            /*Email*/
            , CAST(emailaddress1 AS NVARCHAR(256)) AS [EmailPrimary]
            , CAST(emailaddress2 AS NVARCHAR(256)) AS [Emailtwo]
...
            , CASE WHEN z.new_ssbcrmsystemssidwinner IS NULL OR z.new_ssbcrmsystemssidwinnersourcesystem IS NULL THEN NULL
				...
				ELSE NULL END 
					 AS[ExtAttribute6] --nvarchar(100)
            , CASE WHEN z.parentcustomerid IS NOT NULL AND z.parentcustomeridtype = 'account' THEN z.parentcustomerid ELSE NULL END AS[ExtAttribute7] --nvarchar(100)
            , NULL AS[ExtAttribute8] --nvarchar(100)
            , NULL AS[ExtAttribute9] --nvarchar(100) 
            , RTRIM(CAST(z.contactid AS NVARCHAR(50))) AS[ExtAttribute10] --nvarchar(1000)
 
...
            , CASE WHEN str_number LIKE '%|%'
				THEN  (SELECT value FROM [dbo].[fn_Split](str_number,'| ') WHERE position = 1)
						 WHEN (SELECT value FROM [dbo].[fn_Split](str_number,',') WHERE position = 1) IS NOT NULL
			THEN (SELECT value FROM [dbo].[fn_Split](str_number,',') WHERE position = 1)
				END AS [AccountId]
            , 0 AS IsBusiness
...
             ,RTRIM(CASE WHEN str_number LIKE '%|%' AND tm.ssid IS NOT NULL
				THEN 'TM-' + (SELECT value FROM [dbo].[fn_Split](str_number,'| ') WHERE position = 1)
				 WHEN (SELECT value FROM [dbo].[fn_Split](str_number,',') WHERE position = 1) IS NOT NULL AND  tm.ssid IS NOT NULL
				THEN 'TM-' + (SELECT value FROM [dbo].[fn_Split](str_number,',') WHERE position = 1)
...
                                            See Writup for details.*/
			, CASE WHEN z.str_number IS NOT NULL OR z.str_number <> '' THEN GETDATE() ELSE NULL END AS matchkey_updatedate
        --SELECT COUNT(*)           
        FROM Ducks_reporting.prodcopy.contact z WITH (NOLOCK) 
		LEFT JOIN dbo.vw_dimcustomer dc WITH (NOLOCK) ON CAST(z.contactid AS NVARCHAR(100)) = dc.ssid 
        LEFT JOIN dbo.vw_dimcustomer tm WITH (NOLOCK)ON tm.ssid =  z.new_ssbcrmsystemssidwinner AND tm.SourceSystem = 'TM' AND tm.CustomerType = 'Primary'
        WHERE 1=1
...
     AND z.modifiedon > '2018-04-11 01:46:50.000'
) a
 
)
