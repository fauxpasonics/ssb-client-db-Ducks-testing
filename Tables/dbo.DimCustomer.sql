...
GO
	...
	CREATE TRIGGER [dbo].[extattribute6_updated] 
	ON [dbo].[DimCustomer] 
	AFTER UPDATE 
	AS  
	BEGIN 
	IF UPDATE(ExtAttribute6)  
		BEGIN 
		UPDATE dbo.DimCustomer 
		SET matchkey_updatedate = CURRENT_TIMESTAMP 
		FROM dbo.DimCustomer a 
...
GO
CREATE NONCLUSTERED INDEX [IX_DimCustomer_AccountId_Inc] ON [dbo].[DimCustomer] ([AccountId]) INCLUDE ([CustomerType], [SourceSystem], [UpdatedDate]) WITH (DATA_COMPRESSION = PAGE)
GO
...
GO
CREATE NONCLUSTERED INDEX [IX_Dimcustomer_SourceSystem_AccountID] ON [dbo].[DimCustomer] ([SourceSystem]) INCLUDE ([AccountId])
GO
